...
#let new-lecture = make-lecture-fn("ais")
#let new-module = make-module-fn("ais")
...
#pagebreak()
== #new-lecture("Архитектурные стили и оценка продуктивности слоёв")
...
#figure(
  ...
  align(center)[
    #table(
      columns: (1fr, auto, 1.3fr, auto, 1fr),
      stroke: none,
      align: (center, center, center, center, center),

      block(width: 100%, stroke: 1pt, inset: 8pt, radius: 4pt)[
        *Входной контур (API)* \
        #text(size: 9pt)[HTTP/gRPC-контроллеры \ обработчики запросов]
      ],
      [→],
      block(width: 100%, stroke: 1.2pt, inset: 10pt, radius: 5pt)[
        *Ядро системы* \
        #text(size: 9pt)[Доменные сущности \ и сценарии применения]
      ],
      [←],
      block(width: 100%, stroke: 1pt, inset: 8pt, radius: 4pt)[
        *Выходной контур (инфраструктура)* \
        #text(size: 9pt)[База данных, брокеры \ внешние системы]
      ],
    )
    #v(0.4em)
    #text(size: 9pt)[Зависимости направлены к центру: внешний код зависит от бизнес-ядра.]
  ],
  caption: [Домен в центре, внешние контуры по сторонам]
)
...
#pagebreak()
== #new-lecture("Сетевая модель и контракты взаимодействия")
...
#figure(
  ...
  table(
    ...
    [7], [Прикладной], [Семантика сообщений для приложения], [HTTP, DNS, SMTP, FTP],
    [6], [Представления], [Формат и кодировка данных, шифрование], [TLS, JSON, XML, gzip],
    [5], [Сеансовый], [Установление, поддержание и завершение сессий], [TLS handshake, RPC-сессии],
    [4], [Транспортный], [Надёжная доставка потока данных конкретному процессу], [TCP, UDP],
    [3], [Сетевой], [Адресация и маршрутизация между сетями], [IP, ICMP],
    [2], [Канальный], [Доставка кадров в локальной сети], [Ethernet, Wi-Fi (802.11)],
    [1], [Физический], [Передача битов по физической среде], [Медный кабель, оптика, радио],
  ),
...
+ *Разбиваем биты на кадры* (L2 — канальный). Непрерывный поток битов бесполезен — неясно, где начало сообщения и кому оно адресовано. Договариваемся разбивать поток на *кадры* (frames): в начале каждого кадра — преамбула (фиксированная последовательность битов для синхронизации), затем MAC-адрес получателя (`AA:BB:CC:DD:EE:FF`), MAC-адрес отправителя, длина, полезная нагрузка и контрольная сумма. Сетевая карта читает MAC-адрес — если это её адрес, принимает кадр, иначе игнорирует. \
...
+ *Доставляем данные конкретной программе* (L4 — транспортный). На одном компьютере работают десятки процессов: веб-сервер, база данных, почтовый клиент. IP доставил пакет на машину, но кому именно? Договариваемся о *портах* — числовых идентификаторах (0--65535). TCP добавляет к каждому сегменту порт отправителя и получателя (например, `:443`), а также порядковый номер, подтверждение доставки и контрольную сумму. Если сегмент потерялся — TCP замечает пропуск в номерах и запрашивает повторную отправку. \
...
#figure(
  ...
  table(
    ...
    [L1 — физический],
    [Электрический сигнал \ `+V −V −V +V +V −V −V +V`],
    [Поток битов \ `10011001`],

    [L2 — канальный],
    [Поток битов `10011001...`],
    [Кадр \ `[dst: AA:BB:CC:DD:EE:FF]` \ `[src: 11:22:33:44:55:66]` \ `[данные][CRC]`],

    [L3 — сетевой],
    [Кадр с MAC-адресами \ и данными],
    [IP-пакет \ `[src: 192.168.1.10]` \ `[dst: 93.184.216.34]` \ `[данные]`],

    [L4 — транспортный],
    [IP-пакеты с данными],
    [TCP разбивает данные на сегменты \ `[src port: 51234][dst port: 443]` \ `[seq:1][данные]`, `[seq:2][данные]`,...\ и собирает их в упорядоченный \ поток байтов для процесса \ на порту `443`],

    [L5/L6 — сеансовый, \ представления],
    [Поток байтов \ `47 45 54 20 2F 75 73 ...` \ (это байты `GET /us...`, \ но L5 не знает что это)],
    [Если HTTPS (TLS): зашифрованный \ поток `17 03 03 00 1A 8b 4e ...` \ Если HTTP без TLS: данные \ проходят как есть, без шифрования],

    [L7 — прикладной],
    [Расшифрованный текст \ `GET /users/42 HTTP/1.1\r\n` \ `Host: api.example.com\r\n`],
    [Запрос: `GET /users/42 HTTP/1.1` \ Ответ: `200 OK` \ `{"id": 42, "name": "Alice"}`],
  ),
  caption: [Вход и выход каждого уровня OSI]
)
...
#pagebreak()
== #new-lecture("Протоколы взаимодействия: HTTP, REST, gRPC, GraphQL, WebSocket")
...
#pagebreak()
== #new-lecture("Синхронные и асинхронные модели взаимодействия сервисов")
...
Когда сервис делает *синхронный* вызов по схеме «запрос-ответ» и ждёт результат, он напрямую зависит от доступности и скорости #term("downstream")-сервиса. Практически это означает три базовых класса отказов:
...
- *Каскадная деградация:* медленный или недоступный #term("downstream") удерживает ресурсы #term("upstream")-сервисов (пулы соединений, воркеры, потоки), снижая общую пропускную способность системы.
...
Если эти параметры не заданы, зависший #term("downstream") начинает удерживать поток/воркер, и при росте нагрузки это быстро приводит к исчерпанию ресурсов.
...
*Следствие.* Возникает каскадная деградация: #term("upstream") теряет ресурсы на заведомо безуспешные вызовы.
...
=== Связь метрик: от #term("dau") к #term("qps")

Ключевая задача — перевести бизнес-метрики (#abbr("dau")) в технические (#abbr("qps"), #term("iops")).
...
LSM-дерево оптимизировано под запись, но что если нужна *быстрая точечная выборка* (чтение одной записи по ключу)? Для этого существует *B-дерево* — классическая структура индексирования, используемая в PostgreSQL, MySQL, Oracle, SQLite.

Ключевая идея: группировать данные в узлы размером с дисковый блок (4–16 КБ). Один узел содержит сотни ключей → высокий branching factor → малая глубина. Для поиска среди миллиарда записей достаточно 3–4 чтений с диска ($log_1000 (10^9) approx 3$).

Компромисс: быстрое чтение, но медленная запись — модификация требует чтения узла, изменения и записи обратно (read-modify-write).

=== Структура и B-фактор

*B-фактор* — максимальное количество указателей на детей в узле (максимум $B-1$ ключей).

Структура узла:
- Ключи: $[k_1, k_2, ..., k_(B-1)]$ в отсортированном порядке
- Указатели на детей: $[p_0, p_1, ..., p_B]$, где $p_i$ → поддерево с ключами в $[k_i, k_(i+1))$
- Листовой узел содержит значения вместо указателей на поддеревья
- Каждый узел = одно дисковое обращение

*Выбор B:* узел должен поместиться в один блок диска. Блок 4 КБ, пара (ключ, указатель) ~10 байт → $B approx 400 dash 500$. При $B = 500$ глубина для 1 млн записей — всего 3–4 уровня.

*Инварианты:*
- Все листья на одной глубине $h$
- Внутренний узел содержит от $ceil(B/2)$ до $B-1$ ключей
...
- Если родитель тоже переполнен — рекурсивно разделяем вверх
- Корень может вырасти → высота дерева увеличивается на 1

=== Сложность операций

- Create -- *$cal(O)(log_B n)$*
  - Поиск листового узла: *$cal(O)(log_B n)$* дисковых обращений
  - Вставка в лист: *$cal(O)(1)$*
  - При переполнении: разделение + поднятие медианы — до *$cal(O)(log_B n)$* обращений вверх по дереву

- Read -- *$cal(O)(log_B n)$*
  - Бинарный поиск внутри узла: *$cal(O)(log B)$* — в памяти, быстро
  - Переход к дочернему узлу: одно дисковое обращение
  - Повторение до листа: максимум *$cal(O)(log_B n)$* обращений

- Update -- *$cal(O)(log_B n)$*
  - Поиск: *$cal(O)(log_B n)$*
  - Обновление значения in-place

- Delete -- *$cal(O)(log_B n)$*
  - Поиск и удаление ключа: *$cal(O)(log_B n)$*
  - При недостаточном количестве ключей: слияние с соседним узлом или перемещение ключей

- Range Query -- *$cal(O)(log_B n + R/B)$*, $R$ — размер результата
  - Поиск левой границы: *$cal(O)(log_B n)$*
  - Последовательное сканирование листьев (локальность: листья рядом на диске)

=== Сравнение с LSM и Hash

*B-tree vs LSM:*
- *Чтение:* B-tree — $cal(O)(log_B n)$ обращений к диску. LSM — нужно проверить memtable + несколько SSTable (Bloom-фильтры ускоряют, но не гарантируют)
- *Запись:* LSM быстрее — append-only в memtable, без read-modify-write. B-tree — изменение узла на диске + WAL
- *Стабильность:* B-tree — предсказуемая задержка, нет фоновой компакции. LSM — периодические всплески из-за compaction
- *Итого:* B-tree для OLTP с требованиями к латентности чтения, LSM для write-heavy и time-series

*B-tree vs Hash:*

Hash-индекс использует хеш-функцию: $h(k) arrow.r "bucket"$ за $cal(O)(1)$.

- *Equality lookup:* Hash $cal(O)(1)$ — быстрее B-tree $cal(O)(log_B n)$
- *Range, sort, prefix:* Hash *не поддерживает* — хеш-функция теряет порядок ключей. Из $h(5) = 42$ и $h(10) = 17$ нельзя определить, что $5 < 10$
- *Итого:* Hash — только для equality (`=`, `IN`), например lookup по email или session_id. B-tree — универсальный выбор по умолчанию

*В PostgreSQL:* `CREATE INDEX ... USING hash(col)`. До версии 10 hash-индексы не писались в WAL (не crash-safe). С 10+ полноценно поддерживаются.

=== Практическое применение

Хорошо подходит:
- Случайные чтения по ключу с жёсткими требованиями к задержке
- Смешанные read/write нагрузки, #term("oltp") системы
- Реляционные БД (PostgreSQL, MySQL/InnoDB), файловые системы

Плохо подходит:
- Write-heavy (вставка вызывает дробление узлов, read-modify-write)
- Последовательные вставки, где важна пропускная способность
- Стриминг событий — для этого лучше LSM
...
Модуль содержит пять комплексных практических заданий, каждое из которых охватывает несколько лекций.
...
#pagebreak()
=== Практика 1: Проектирование API и взаимодействия (Лекции 1-4: компоненты, слои, SOLID, протоколы)
...
В этой практике принцип простой: сначала проектируем, потом кодим. Цель — выбрать осмысленный бизнес-воркфлоу по варианту и заранее спроектировать API, слои и use cases.
...
Единообразный путь данных означает, что любой запрос проходит одну и ту же цепочку слоёв. Без этого каждый новый endpoint изобретает собственную структуру — особенно заметно при работе с #term("llm"):
...
#table(
  ...
  fx (Uber)\
  ручной DI в main.go],
  [gRPC из коробки, OpenAPI yaml first, sqlc для типобезопасного SQL],

  [Python],
  [FastAPI (OpenAPI из коробки)\
  Django REST Framework\
  Flask],
  [FastAPI Depends (встроен)\
  dependency-injector],
  [Быстрый старт, автодокументация API в FastAPI],
  ...
  Koin (Kotlin)],
  [Зрелая экосистема, Spring покрывает всё],

  [C\#],
  [ASP.NET Core Minimal API\
  ASP.NET Core MVC],
  [встроен в ASP.NET Core\
  Autofac],
  [DI из коробки, сильная типизация],
  ...
  [Fullstack на одном языке, Next.js для SSR],

  [Elixir],
  [Phoenix (LiveView / JSON API)],
  [behaviours + конфигурация],
  [Fullstack с real-time из коробки],
)
...
#pagebreak()
   *Пример эталонной схемы (Clean Architecture):*
   ...
   #figure(
     image("assets/clean-architecture.svg", width: 100%),
     caption: [Блок-схема по слоям: сценарии зависят от портов (→), реализации реализуют порты (←). Один порт — несколько сценариев: «получить по ID» используют три сценария.],
     supplement: [Схема],
   )

   #text(size: 9pt)[
     На схеме показан усложнённый, но типовой сценарий: несколько входных протоколов и несколько внешних хранилищ/сервисов. Стрелки направлены к центральному слою use case и отражают правило зависимостей Clean Architecture (зависимости направлены внутрь). Для этой работы минимальное требование: не менее 4 компонентов в каждом слое — входные обработчики, use case и порты. Требования по авторизации в этой практике отсутствуют: фокус на архитектуре и бизнес-логике.
   ]
...
#pagebreak()
=== Практика 2: Реализация спроектированного приложения (Лекции 1-4: реализация, тесты, отчёт)

=== Предисловие

Во второй части реализуется проект из Практики 1. Главная цель — превратить архитектурную схему в работающий код и убедиться, что *маппинг между рисунком и кодом* прослеживается явно: каждый блок на схеме соответствует конкретному пакету/модулю, каждая стрелка — конкретному `import`/`require`/`using`.

*Подключение базы данных.* Вы можете подключить PostgreSQL, SQLite или любую другую СУБД — но это *не обязательно*. In-memory реализации (`map`/`slice`/`dict`) достаточно. Смысл практики не в настройке БД, а в том, чтобы зависимости в коде соответствовали зависимостям на схеме. Если ваша архитектура спроектирована правильно, замена in-memory репозитория на PostgreSQL потребует изменения ровно одного файла — реализации интерфейса.
...
- Зависит ли use case от интерфейса репозитория, а не от конкретной реализации.
- Нет ли "коротких путей" (handler напрямую обращается к хранилищу, минуя use case).

=== Задание

1. *Реализуйте слои из вашего проекта:* domain, use cases, adapters, infrastructure.
2. *Хранение данных:* in-memory (`map`/`slice`/`dict`) или реальная СУБД — на ваш выбор. Если выбрали СУБД, убедитесь, что use cases зависят от интерфейса репозитория, а не от конкретной реализации (ORM, SQL-драйвер и т.д.).
...
4. *Интеграционные тесты (необязательно, но приветствуются):*
   - Если пишете тесты — минимум один тест на полный воркфлоу из нескольких шагов.
...
+ *Введение.* Краткое описание продукта и воркфлоу (из Практики 1). Если что-то изменилось — укажите что и почему.
...
+ *Маппинг «схема → код».* Таблица, связывающая каждый блок на схеме с конкретным файлом/пакетом в проекте:

  #table(
    columns: (1.2fr, 1.2fr, 2fr, 1.5fr),
    align: (left + top, left + top, left + top, left + top),
    stroke: 0.5pt,
    inset: 6pt,
    table.header(
...
+ *Проверка endpoint'ов.* Для каждого endpoint — скриншот из Insomnia/Postman: запрос и ответ. Для мутирующих endpoint'ов — дополнительно скриншот сценария с ошибкой. Если написаны интеграционные тесты — опишите сценарии и как их запустить.
...
2. Реализации всех репозиториев (in-memory или СУБД).
...
- $L$ [req] — среднее число запросов, обрабатываемых одновременно (concurrency). $L = N_"cores" times eta$, где $eta approx 0.3 dash 0.5$ — коэффициент полезной работы (остальное: context switching, lock contention, GC)
- $W$ [s] — среднее время обработки одного запроса (суммарное время транзакции из EXPLAIN ANALYZE)

Шаги:
1. Из EXPLAIN ANALYZE (задача 3) возьмите суммарное $W$ транзакции
2. Вычислите $L$ для вашего целевого железа
...
#table(
  ...
  table.header([*Запрос*], [*Узел*], [*Rows*], [*Buffers*], [*Время*]),
...
#table(
  ...
  stroke: 0.5pt,
  align: (left, center, center, center, center),
  table.header([*Индекс*], [*Point: узел*], [*Point: время*], [*Range: узел*], [*Range: время*]),
...
*Целевое железо:* 8 ядер, коэффициент эффективности 0.4:
$ L = 8 times 0.4 = 3.2 $

*Little's Law:*
$ "max RPS" = L / W = 3.2 / 0.0009 approx 3556 $
...
$ "DAU" = frac(3556 times 43200, 20 times 2.5) = frac(153619200, 50) approx 3072000 $
...
#pagebreak()
=== Практика 4: Специализированные индексы и поиск (Лекции 10-13: Inverted Index, векторный поиск, геопоиск)
...
#pagebreak()
=== Практика 5: Операционные компоненты + OLAP (Лекции 14-16: балансировка, авторизация, мониторинг, ETL)
