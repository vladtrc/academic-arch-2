...
Теперь рассмотрим архитектуру на уровне всей системы: направление зависимостей, оценку продуктивности слоёв и выбор критерия оптимизации.
...
Ключевой практический вывод: большинство современных прикладных интеграций строится *поверх HTTP*. REST использует HTTP напрямую, GraphQL обычно передаётся через HTTP, WebSocket устанавливается через механизм обновления протокола HTTP (`Upgrade`), а gRPC работает поверх HTTP/2 (gRPC-Web --- через HTTP-прокси в браузерной среде).

Поэтому, когда компоненты расположены в разных процессах или на разных машинах, их взаимодействие нельзя трактовать как обычный вызов функции. Требуется определить *контракт сетевого взаимодействия*: формат сообщений, правила обработки ошибок, таймауты, ретраи и семантику ответов.
