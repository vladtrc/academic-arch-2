...
REST, gRPC, GraphQL и WebSocket чаще всего используются как *синхронное* взаимодействие: клиент отправляет запрос и ждёт ответ.
Это удобно, когда нужен мгновенный результат, но именно здесь появляются основные эксплуатационные риски распределённых систем.

=== Цена синхронного взаимодействия и альтернатива
Когда сервис делает *синхронный* вызов (request/response) и ждёт ответ, он напрямую зависит от доступности и скорости downstream-сервиса. В этой модели любой вызов может завершиться частично:
- запрос дошёл, но ответ потерялся;
- обработчик выполнился, но подтверждение не сохранилось;
- downstream завис, и цепочка начала накапливать ожидания.

Чтобы такая синхронная схема оставалась управляемой, применяют набор базовых правил:
- *Timeouts* --- у каждого сетевого вызова есть верхняя граница ожидания. Без таймаутов воркеры и потоки постепенно «залипают» на зависших зависимостях.
- *Retries* --- повторные попытки делаются только для временных ошибок (например, `429`, `503`, сетевые сбои) и только с backoff + jitter, чтобы не усилить перегрузку.
- *Idempotency* --- повтор одного и того же запроса/сообщения не должен создавать дубликаты бизнес-операций. Обычно используется idempotency key и таблица обработанных операций.
- *Circuit Breaker* --- при серии ошибок зависимость временно «размыкается»: новые вызовы сразу отклоняются или переводятся на fallback, чтобы защитить систему от каскадного отказа.
- *Deadline Propagation* --- общий дедлайн запроса передаётся по всей цепочке сервисов. Если клиенту осталось 200 мс, внутренние вызовы не должны запускаться с таймаутом в 2 секунды.

Практическое следствие: retry без идемпотентности опасен, а timeout без circuit breaker недостаточен. Эти механизмы работают как единый контур устойчивости.

Альтернатива --- *асинхронное взаимодействие*: сервис не блокируется в ожидании ответа, а публикует событие или задачу в брокер/очередь. Это снижает связность по времени и лучше переживает пики нагрузки, но добавляет eventual consistency, необходимость дедупликации и наблюдения за lag очередей.
...
=== Зачем асинхронность
- Развязка сервисов: producer не ждёт, пока consumer завершит работу
- Устойчивость к пикам нагрузки: очередь буферизует всплески
- Повышение надёжности: повторная доставка, retry, DLQ
- Улучшение UX: пользователь получает быстрый `202 Accepted`, тяжёлая работа уходит в фон

=== Брокеры сообщений и очереди
- *Kafka* — журнал событий (event log), высокая пропускная способность, partition'ы, consumer groups
...
- Гарантии доставки: at-most-once, at-least-once, effectively-once (через идемпотентность)
- Eventual consistency: состояние сходится не мгновенно, а через поток событий
- Saga-подход: длинные бизнес-процессы с компенсирующими действиями
...
2. In-memory реализации всех репозиториев.
...
#pagebreak()
=== Практика 3: OLTP-система и расчёт нагрузки (Лекции 5-9)
