...
=== Зачем нужны сетевые протоколы взаимодействия
...
==== От физического канала к HTTP

На нижних уровнях сеть решает задачу *доставки*: как сигнал проходит по среде, как кадры и пакеты достигают нужного узла, как поток данных попадает в конкретный процесс.
На верхних уровнях решается задача *смысла*: в каком формате передаются данные, как описывается операция, как интерпретировать успешный и ошибочный результат.
...
Важно различать транспортный протокол и прикладной контракт. HTTP задаёт правила обмена (метод, заголовки, статус, тело сообщения), но сам по себе не определяет бизнес-семантику полезной нагрузки (`payload`).
Семантика и формат данных определяются прикладным протоколом и договорённостями API.
