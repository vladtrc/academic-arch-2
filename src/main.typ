...
*Полезные константы:* секунд в сутках $approx 10^5$ (точно: 86400), в месяце $approx 2.5 times 10^6$, в году $approx 3 times 10^7$.

*Формула DAU → RPS:*
$ "RPS"_"средний" = ("DAU" times "действий на пользователя") / 10^5 $

*Пример:* 100K DAU, 20 действий/день → $100000 times 20 / 100000 = 20$ RPS (средний).

*Учёт пиков:* нагрузка распределена неравномерно. Активные часы — 12 из 24, пиковые — 4 часа с нагрузкой в 3× от среднего.

$ "RPS"_"пиковый" approx "RPS"_"средний" times 3 $
...
*Источник:* таблица основана на данных Jeff Dean (Google) и Peter Norvig, актуализированных Colin Scott. См. #link("https://gist.github.com/jboner/2841832")[Latency Numbers Every Programmer Should Know] и #link("https://colin-scott.github.io/personal_website/research/interactive_latency.html")[Interactive Latency].
...
$ lambda = L / W = 28 / 0.001 = 28000 "RPS (теоретический максимум)" $
...
$ lambda_"реальный" = 28000 times 0.4 approx 11000 "RPS" $

*Декомпозиция запроса* — ключ к оценке $W$. Разбиваем HTTP-запрос на операции:
- CPU (parsing, сериализация): 10–100 мкс
- B-tree read: глубина × время чтения страницы (см. следующие лекции)
...
#pagebreak()
=== Практика 1: Архитектура приложения (Лекции 1-4: компоненты, слои, SOLID, протоколы)
...
#pagebreak()
=== Практика 2: OLTP-система и расчёт нагрузки (Лекции 5-9)
...
Укажите источник, конфигурацию железа и полученный #term("tps") — это ваш baseline.

*Задача 4. Расчёт максимального RPS для вашего endpoint'а*

*Шаг 4.1. Нормализация нагрузки*

Подсчитайте #term("dml")-операции в вашем endpoint'е и сравните с эталоном:
...
#pagebreak()
=== Практика 4: Операционные компоненты + OLAP (Лекции 14-16: балансировка, авторизация, мониторинг, ETL)
